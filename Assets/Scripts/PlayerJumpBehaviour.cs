...
public class PlayerJumpBehaviour : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        else // only happens while we are jumping
        {
            ...
            if (playerTransform.position.y > -0.999 && tempAccelleration < 0) // failsafe to make zpos 0 if falling and close to ground
            {
                playerTransform.position = new Vector3 (playerTransform.position.x,playerTransform.position.y,Mathf.Abs(playerTransform.position.y - playerTransform.position.y));
                isGrounded = true;
            }
